...
symmetric stage. Use compact icon-first controls with hover label reveal (label slides
toward the center). Move copyright and legal links to bottom center. Fix glitch split
animations so the full control hit area stays clickable. Per-track streaming links and a
chronological track catalog are out of scope (IDEA-021). Mobile composition stays on
IDEA-013."
...
| Label reveal        | At rest, controls show only an icon (emoji, pictogram, or icon-font glyph). On hover or keyboard-visible focus, the readable label animates from the control toward the horizontal center of the viewport so meaning is clear without permanent text chrome |
| Open panels         | Panel bodies still attach to the periphery and scroll internally; they do not occupy the center stage                                                                                                                                                       |
| Bottom center       | Single footer cluster: copyright, Impressum, Datenschutzerklärung                                                                                                                                                                                           |
| Mute                | Stays reachable but grouped so it does not dominate the right edge; placement chosen to preserve symmetry                                                                                                                                                   |
...
keyboard-focus a control, its human-readable label appears and animates toward the
horizontal center so they immediately understand what the icon means — without permanent
labels consuming space.
...
and label motion travels toward the viewport center before settling or fading.

**Acceptance Scenarios**:

1. **Given** motion is allowed and a HUD control is visible, **When** the visitor hovers
  it, **Then** a readable label appears and moves from the control toward the horizontal
   center of the viewport.
2. **Given** a HUD control has a label, **When** the visitor moves keyboard-visible focus
  onto it, **Then** the same label reveal occurs (not hover-only).
3. **Given** reduced motion is preferred, **When** the visitor focuses or hovers a
  control, **Then** the label appears without traveling motion (instant or minimal fade
   near the control is acceptable).
4. **Given** a control’s label text is changed in chrome content, **When** the site is
...
- **Very wide monitors**: Label slide toward center must not collide unreadably with open
panel bodies; labels may fade or stop at a defined center band.
...
`hudGlitch`.
- **No scripting**: Icons still visible; panels open via native disclosure; label slide
may degrade to static visible text near icons (documented degradation).
...
- **FR-004**: When motion is allowed, label reveal MUST include animation of the label
from the control toward the horizontal center of the viewport.
...
- **Label reveal**: Transient UI state showing a control’s label, optionally animated
toward viewport center when motion is allowed.
...
- **SC-006**: With reduced motion preferred, label reveal completes without center-travel
animation and remains readable.
...
- Label slide “toward the middle” means horizontal center of the viewport; vertical
position may be fixed near the control row or a defined label band (plan decision).
