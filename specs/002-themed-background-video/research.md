...
- **Decision**: One small bundled script attached to `MuteControl` (and shared fallback
  wiring): toggle `muted` / `volume`, update accessible name/pressed state, set fallback
  state on error/play rejection. No analytics, no frameworks, no hydration libraries.
...
- **Alternatives considered**: React/Solid island (rejected: dependency weight); zero JS
  with checkbox mute (weak a11y and no failure path).
