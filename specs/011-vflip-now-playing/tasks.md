...
**Organization**: P1 stories — US1 inline info → US3 mute-in-box → US5 shuffle → US6 loop.
US2 (in-drawer lyrics) **deferred / out of v1**. US4 sync; US7 regression.
...
**Goal**: Open V-Flip shows panel title + track list with **inline info** on the selected row; Track info dock icon is gone

**Independent Test**: quickstart.md Scenario 1 — open V-Flip, see panel title + inline info on active row; right dock has no Track info icon

### Implementation for User Story 1

- [X] T012 [US1] Widen open jukebox shell CSS in `src/components/Jukebox.astro` to `min(22rem × --hud-scale, …)` and taller scroll body per UI contract
- [X] T013 [US1] Add open drawer **panel title** (`jukeboxPanelTitle`) in `src/components/Jukebox.astro` — not a separate active-track header beside vinyl
- [X] T014 [US1] Embed `TrackInfoPanel` **inline in selected list row** in `src/components/Jukebox.astro`; sync via `[data-track-info-for]`
- [X] T015 [US1] Remove Track info panel entry from `src/components/StagePanels.astro` right-dock list
- [X] T016 [US1] Update `syncStageUi` in `src/lib/stage-switch.ts` for list `aria-pressed` + inline track-info visibility on stage select
- [ ] T017 [US1] Manually walk quickstart.md Scenario 1

**Checkpoint**: MVP — visitor opens V-Flip once and sees what is playing + track info

---

## Phase 4: User Story 2 - Lyrics in V-Flip (Priority: deferred / out of v1)

**Goal**: Lyrics **do not** appear in open V-Flip. Lyrics dock icon removed. Content files unchanged.

**Independent Test**: quickstart.md Scenario 1 — no lyrics block; right dock has no Lyrics icon

### Implementation for User Story 2

- [X] T018 [US2] **Do not** mount `LyricsPanel` in jukebox drawer (deferred scope)
- [X] T019 [US2] Remove Lyrics panel entry from `src/components/StagePanels.astro` (right dock: About → Discography → Tour only)
- [X] T020 [US2] Confirm drawer body is panel title + track list only (inline info on selected row)
- [ ] T021 [US2] Manually verify no lyrics in drawer; right dock has no Lyrics icon
...
- [X] T022 [US3] Mount `MuteControl` inside `[data-jukebox]` **toolbar** in `src/components/Jukebox.astro` (vinyl → shuffle → loop → mute)
...
- [X] T027 [US5] Add shuffle button in **toolbar** `src/components/Jukebox.astro` (`aria-pressed`, chrome labels, glitch when live)
...
- [X] T030 [US5] Implement hop path in `src/lib/stage-switch.ts` — `pickOtherId` with eligible pool when unmuted; crossfade via `resolveThemeHandoff()` (`smooth` 1000ms / `glitch` 720ms)
- [X] T031 [US5] Add `html[data-stage-crossfade='smooth'|'glitch']` transitions in `src/styles/themes.css`; reduced-motion = instant swap
...
- [X] T034 [US6] Add loop button in **toolbar** `src/components/Jukebox.astro` (`aria-pressed`, chrome labels)
...
**Goal**: Manual pick updates list selection, inline info, theme, mute visibility without reload; unmute survives
...
- [X] T039 [US4] Verify `syncStageUi` updates `[data-track-info-for]`, list `aria-pressed`, mute slot visibility on every select/hop
...
- [ ] T042 [US7] Keyboard pass per UI contract — vinyl → shuffle → loop → mute → list → right dock → footer
...
- [X] T044 [P] Update `docs/artist-guide.md` — inline track info in V-Flip; toolbar shuffle/loop; no in-drawer lyrics; shuffle timing; chrome keys
...
- **US1 → US3**: Sequential shell composition (same `Jukebox.astro`)
- **US2**: Deferred — dock icon removed only; no in-drawer lyrics
...
|-------|------------|-------|
| US1 | Phase 2 | MVP — panel title + inline info in open V-Flip |
| US2 | US1 | Dock Lyrics icon removed; in-drawer lyrics deferred |
...
2. Phase 3 US1 — open player with panel title + inline info; Track info icon gone
3. **STOP** — validate Scenario 1
4. Then US3 (mute-in-box) → US5/US6 (transport) before full playback polish

### Incremental Delivery

1. Foundation → US1 (MVP) → US3 (mute box) → US5 (shuffle) → US6 (loop) → US4 → US7
2. Polish: artist guide + contract pointers + full quickstart

### Suggested MVP Scope

**US1 only**: visitor opens V-Flip and sees panel title + inline track info on the
active row; Track info removed from right dock.
