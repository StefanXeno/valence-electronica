...
(mute, reduced motion, legal overlay) and `003` glitch. Glitch on HUD chrome is
**Nightmare-only** (`html[data-theme='nightmare-crimson']`).
...
| Socials | Persistent compact chrome: existing channels as **equal-sized icons only** (~2.5rem × `--hud-scale` circles). Top-right. Visible labels off; names via `aria-label`. Active links: new tab + `glitch-hit` (Nightmare). Inactive: non-link icon + coming-soon accessible name. |
| Jukebox | Persistent compact chrome. Bottom-left. **Collapsed vinyl-record control** at rest (same family as mute). Song list only while open (`<details>`). Always present, including with one entry. |
...
| Nightmare + video sources | Looping `<video>` plays (muted until unmute). Mute control shown. |
| Other theme, or no sources | Pause/clear video; static poster; `data-bg-state=fallback`; mute hidden. |
...
## Glitch (Nightmare only)

Off Nightmare, no glitch animations (including leftover classes).
...
- Glitch on themes other than Nightmare
